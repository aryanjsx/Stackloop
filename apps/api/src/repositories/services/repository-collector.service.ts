...
import { QueueProducer, RepositoryRecord, RepositoryRepository } from '../types.js';

export interface RepositoryCollectorDependencies {
...
export interface CollectRepositoryResult {
  created: boolean;
  repository: RepositoryRecord;
  summaryQueued: boolean;
  searchIndexQueued: boolean;
}

export class RepositoryCollectorService {
  constructor(private readonly deps: RepositoryCollectorDependencies) {}

  async collectRepository(input: CollectRepositoryInput): Promise<CollectRepositoryResult> {
  ...
  async collectRepositoryBatch(inputs: CollectRepositoryInput[]): Promise<CollectRepositoryResult[]> {
    const results: CollectRepositoryResult[] = [];
  ...
  private shouldUpdate(existing: RepositoryRecord, metadata: any): boolean {
    return Boolean(
      existing.description !== (metadata.description ?? undefined) ||
